...
def upgrade() -> None:
    ...
    # ══════════════════════════════════════════════
    if not _tbl_exists(conn, "security_score_config"):
        op.create_table(
    ...
    # ══════════════════════════════════════════════
    if not _tbl_exists(conn, "security_score_snapshots"):
        op.create_table(
    ...
    # Seed default config (skip if already seeded)
    row_exists = conn.execute(sa.text(
        "SELECT COUNT(*) FROM security_score_config WHERE version = 1"
    )).scalar()
    if not row_exists:
        conn.execute(sa.text("""
            INSERT INTO security_score_config (version, is_active, changed_by, change_reason)
            VALUES (1, 1, 'system', 'Initial default configuration')
        """))
