...
Rozszerzenie v2.0 dodaje **8 nowych modułów operacyjnych** oraz **centralny Security Score** oparty na 10 filarach, dając CISO jeden spójny obraz stanu bezpieczeństwa organizacji.
...
**policy_standard_mappings:** policy_id, standard_name (ISO 27001/CIS v8/NIST CSF/SOC 2/RODO/PCI DSS), control_ref (np. "A.9.2.1"), control_description.
...
| 5 | Dojrzałość kontroli (CIS) | 10% | CIS Benchmark (moduł 10) |
...
### 10.3. Formuła filaru "CIS"

```
CIS_Score = overall_maturity_rating × 20 (skala 0–5 → 0–100)
```
...
**security_score_snapshots:** snapshot_date, total_score, pillar_1..10_score, pillar_1..10_weight, config_version, triggered_by ("scheduled"/"manual"/"cis_assessment"/"config_change"), created_by.

Automatycznie: codziennie (cron). Dodatkowo: po ocenie CIS, po zmianie konfiguracji, na żądanie.
...
| Metryka | Wartość |
|---------|---------|
| Nowe moduły | 8 (+1 scoring) |
| Nowe tabele | ~20 |
| Nowe słowniki | 25 |
| Filary Security Score | 10 |
| Nowe endpointy API | ~60 |
| Nowe widoki frontend | 10+ |
...
- **Konfigurowalność** — wagi, progi, parametry zmieniane bez kodu
